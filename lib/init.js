		...
		process.nextTick(function() {
			...
			User.findOne({ $or: [ queryProfileId, queryLoginId ] }, function(err, user) {
				...
				user.set('social.' + provider.name + '.refreshToken', refreshToken);
				 ...
				 if(parsedProfile.raw)
                    user.set('social.' + provider.name + '.raw', parsedProfile.raw);
...
var init = function init() {
	...
	if (!_.keys(this.get('providers')).length) {
		this.__error('No providers configured. You must configure at least one provider.', 'ConfigError');
	}
